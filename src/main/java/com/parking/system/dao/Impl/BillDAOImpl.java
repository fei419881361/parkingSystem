...
public class BillDAOImpl extends AbstractDAOImpl implements BillDAO {
    ...
    public boolean doCreate(Bill vo) throws SQLException {
        ...
        pstmt.setString(6, vo.getOwner_name());
        pstmt.setString(7, vo.getOwner_phone());
        pstmt.setDate(8, new Date(vo.getCreateTime().getTime()));
        return pstmt.executeUpdate() > 0 ? ResponseInfo.SUCCESS : ResponseInfo.FAIL;
    }

    @Override
    public boolean doUpdate(Bill vo) throws SQLException {
        String sql = "UPDATE bill SET park_id = ?,int_time= ?,out_time= ?" +
                ",bill= ?,number=?,owner_phone=?,owner_name=?,createTime=?,updateTime=? WHERE id=?";
        pstmt = super.conn.prepareStatement(sql);
        pstmt.setInt(1, vo.getPark_id());
        pstmt.setString(2, vo.getIn_time());
        pstmt.setString(3, vo.getOut_time());
        ...
        pstmt.setString(7, vo.getOwner_name());
        pstmt.setDate(8, new Date(vo.getUpdateTime().getTime()));
    ...
    public List<Bill> findAllBySplit(String column, String keyWord, Integer currentPage, Integer lineSize) throws SQLException {
        ...
        if (keyWord == null || keyWord.equals("")) {
            sql = "SELECT b.id,b.park_id,b.in_time,b.out_time,b.bill,b.owner_name" +
                    ",b.owner_phone,b.createTime,b.updateTime,b.number FROM bill b LIMIT ?,?";
            pstmt = super.conn.prepareStatement(sql);
            ...
            while (resultSet.next()) {
                ...
                bill.setNumber(resultSet.getString(10));
                bills.add(bill);
            }
        } else {
            sql = "SELECT b.id,b.park_id,b.in_time,b.out_time,b.bill,b.owner_name" +
                    ",b.owner_phone,b.createTime,b.updateTime,b.number FROM bill b WHERE b.id = ? LIMIT ?,? ";
            pstmt = super.conn.prepareStatement(sql);
